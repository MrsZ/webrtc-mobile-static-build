...
def build(target_dir, platform, debug):
    ...
    if platform == 'ios':
        ...
        for cpu in ANDROID_BUILD_CPUS:
            gn_out_dir = 'out/%s-%s' % (build_type, cpu)
            ninja_cmd = 'ninja -C %s webrtc' % gn_out_dir
            sh(ninja_cmd, env)

def package(target_dir, platform, debug):
    ...
    if platform == 'ios':
        ...
        for cpu in ANDROID_BUILD_CPUS:
            ...
            mkdirp(lib_dir)
            gn_out_dir = 'out/%s-%s' % (build_type, cpu)
            print('copy', os.path.join(gn_out_dir, 'obj/webrtc/libwebrtc.a'), lib_dir)
            shutil.copy(os.path.join(gn_out_dir, 'obj/webrtc/libwebrtc.a'), lib_dir)

        include_dir = os.path.join(build_dir, 'include')    
        def ig_f(dir, files):
            return [f for f in files if not (os.path.isdir(os.path.join(dir, f)) or f.endswith('.h'))]

        shutil.copytree(os.path.join('webrtc'), include_dir, ignore=ig_f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--package', help='Package WebRTC in the target directory', action='store_true')
    ...
    if not (args.setup or args.build or args.package):
    ...
    if args.package:
        package(target_dir, platform, args.debug)
        print('WebRTC package for %s completed in %s' % (platform, target_dir))
        sys.exit(0)
